...
async function scrapeData(req, res, next) {
    console.log('beginning Montana scraping process...');

    console.log(req.query);
    if ((req.query.host).indexOf("mtsosfilings") !== -1) {
        var domainLink = "https://www.mtsosfilings.gov/mtsos-master/viewInstance/view.html";
        const keywordName = req.query.keyword; // can search with ID or Name
        ...
        var spooky = new Spooky({
            ...
            spooky.then([{
                ...
                function() {
                    ...
                    this.thenEvaluate(function(term) {
                        ...
                        document.querySelector("button.searchButton").click();
                    }, keyword);
                }
            ]);
            spooky.waitFor(function() {
                // this.captureSelector('asd.png','div.appSearchResultsChildren>div.appRepeater>div.appRepeaterContent');
                //  this.emit('message', 'loading...');
                return (this.exists('div.appSearchResultsChildren .appRepeaterContent'));
            }, function() {
                // then
            ...
            spooky.then([{
                ...
                function() {
                    ...
                    if (this.exists('div.appSearchResultsChildren .appRepeaterContent')) {
                        var info = this.evaluate(function(domainLink) {
                            var table_rows = document.querySelectorAll("div.appSearchResultsChildren .appRepeaterContent .appRepeaterRowContent"); //or better selector
                            return Array.prototype.map.call(table_rows, function(tr) {

                                var table_data = tr.querySelectorAll('div.appMinimalBox .repeaterItemAddresses');
                                var detailData = Array.prototype.map.call(table_data, function(tr) {
                                    var returnObj = {};
                                    if (tr.children.length <= 0) {
                                        ...
                                        returnObj = {
                                            "street_Address": tr.querySelector('div.appAttrValue').textContent
                                        }
                                    }
                                    return returnObj;
                                });
                                ...
                                return {
                                    title: tr.querySelector('div.appRawText').textContent.replace(/\s\s+/g, ''),
                                    business_Name: tr.querySelector('div.appMinimalBox .appMinimalMenu').textContent.replace(/\s\s+/g, ''),
                                    Status: tr.querySelector('div.appMinimalBox .appMinimalAttr .appMinimalValue').textContent.replace(/\s\s+/g, ''),
                                    registration_date: tr.querySelector('div.appMinimalBox .RegistrationDate .appMinimalValue').textContent.replace(/\s\s+/g, ''),
                                    addresses: detailData
                                };
                            });
                        }, domainLink);
